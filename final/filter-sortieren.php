...
<html class="no-js" lang="DE">
    ...
    <body>
        ...
        <section class="allcont filter-system">
                                ...
                                <form class="filter-form" id="sortierenBenchmark" method="post" action="filter-sortieren.php">
                                    ...
                                    <div class="filter-bench filter-div">
                                        ...
                                        <label class="control control-checkbox">
                                            AnTuTu Benchmark<input value="5" name="sortierung" type="radio" class="antutu" checked/>
                                        ...
                                        <label class="control control-checkbox">
                                            DxOMark<input value="6" name="sortierung" type="radio" class="DxOMark" checked/>
                                            <div class="control_indicator"></div>
                                        </label>
                                        <?php
                                            }
            ...
            <?php 
                if ($sortierung == 1) {
                    $abfrage = "SELECT MODELLNAME, MIN_PREIS FROM SMARTPHONE_DATENBLATT ORDER BY MIN_PREIS ASC";
                }
                else if ($sortierung == 2) {
                    $abfrage = "SELECT MODELLNAME, MIN_PREIS FROM SMARTPHONE_DATENBLATT ORDER BY MIN_PREIS DESC";
                }
                else if ($sortierung == 3) {
                    $abfrage = "SELECT MODELLNAME, ERSCHEINUNGSSDATUM_DEU FROM SMARTPHONE_DATENBLATT ORDER BY ERSCHEINUNGSSDATUM_DEU DESC";
                }
                else if ($sortierung == 4) {
                    $abfrage = "SELECT MODELLNAME, ERSCHEINUNGSSDATUM_DEU FROM SMARTPHONE_DATENBLATT ORDER BY ERSCHEINUNGSSDATUM_DEU ASC";
                }
                else if ($sortierung == 5) {
                    // AnTuTu
                    $abfrage = "SELECT MODELLNAME, CPU_SCORE, UX_SCORE, 3D_SCORE, RAM_SCORE FROM ANTUTU_BENCHMARK ORDER BY CPU_SCORE + UX_SCORE + 3D_SCORE + RAM_SCORE DESC";
                }
                else if ($sortierung == 6) {
                    // DxOMark
                    $abfrage = "SELECT MODELLNAME, PHOTO_SCORE, VIDEO_SCORE FROM DXOMARK_BENCHMARK ORDER BY PHOTO_SCORE + VIDEO_SCORE DESC";
                }
                else {
